...
struct BatteryInfo {
    ...
    bool dedicated_update = false;

    std::uint8_t legacy_percentage() const {
        return static_cast<std::uint8_t>(
            (static_cast<unsigned>(raw_status) * 100u + 127u) / 255u);
    }

    bool operator==(const BatteryInfo& other) const {
        return raw_status == other.raw_status &&
               dedicated_update == other.dedicated_update;
    }
};
...
struct WirelessEvent {
    ...
    // Research-only byte 17 from the 29-byte initial announcement.
    // It is deliberately not interpreted as a battery percentage.
    std::optional<std::uint8_t> battery_candidate;

    // Set only for a dedicated 00 00 00 13 XX battery/status packet.
    std::optional<BatteryInfo> battery;

    // Research-only marker for packets whose XUSB bType appears to be 0x09
    // (Battery PID Packet). The exact payload is logged by receiver.cpp.
