...
NSString* const kHIDProfileKey = @"X360Bridge.hidProfile";
...
constexpr std::uint8_t kWirelessTestRumbleStrong = 255;
constexpr std::uint8_t kWirelessTestRumbleWeak = 255;
...
struct WirelessSlotModel {
    ...
    std::size_t battery_update_count = 0;
    std::size_t battery_pid_count = 0;
};
...
NSString* permissionStatusText() {
    switch (virtual_hid_permission_status()) {
        ...
        case VirtualHidPermissionStatus::denied:
            return NSLocalizedString(@"Needs approval", nil);
        case VirtualHidPermissionStatus::unknown:
            return NSLocalizedString(@"Not configured", nil);
...
NSString* batterySummary(const std::optional<BatteryInfo>& battery,
                         const std::optional<std::uint8_t>& candidate,
                         std::size_t updateCount,
                         std::size_t batteryPidCount) {
    if (battery.has_value()) {
        NSString* source = battery->dedicated_update ? NSLocalizedString(@"Dynamic update", nil) : NSLocalizedString(@"Initial report", nil);
        return [NSString stringWithFormat:
            @"%u%% • %@ • raw 0x%02x • dynamic updates %zu • PID 0x09 %zu",
            static_cast<unsigned>(battery->legacy_percentage()),
            source,
            static_cast<unsigned>(battery->raw_status),
            updateCount,
            batteryPidCount];
    }
    if (candidate.has_value()) {
        return [NSString stringWithFormat:
            @"Unknown • initial candidate 0x%02x • dynamic updates %zu • PID 0x09 %zu",
            static_cast<unsigned>(*candidate),
            updateCount,
            batteryPidCount];
    }
    return [NSString stringWithFormat:
        @"Unknown • waiting for battery report • dynamic updates %zu • PID 0x09 %zu",
        updateCount,
        batteryPidCount];
}
...
X360ControllerSnapshot* controllerSnapshot(NSString* identifier, NSString* title, NSString* detail,
                                           ...
                                           const std::optional<BatteryInfo>& battery,
                                           const std::optional<std::uint8_t>& batteryCandidate,
                                           std::size_t batteryUpdateCount,
                                           std::size_t batteryPidCount,
    ...
    snapshot.batteryKnown = battery.has_value();
    snapshot.batteryOnline = battery.has_value();
    snapshot.batteryCharging = NO;
    snapshot.batteryLevel = 255;
    snapshot.batteryPowerMode = 255;
    snapshot.batteryRawFlags = battery.has_value() ? battery->raw_status : 0;
    snapshot.batteryStatus = batterySummary(battery, batteryCandidate, batteryUpdateCount, batteryPidCount);
...
@private
    ...
    BOOL _scanning;
    BOOL _refreshQueued;
    NSMutableArray<NSString*>* _virtualHIDDevices;
}
@end
...
- (instancetype)init {
    ...
    if (self) {
        ...
        _scanning = NO;
        _refreshQueued = NO;
        _virtualHIDDevices = [NSMutableArray array];
        if (![NSUserDefaults.standardUserDefaults objectForKey:kHIDProfileKey]) {
            [NSUserDefaults.standardUserDefaults setInteger:0 forKey:kHIDProfileKey];
        }
    }
    return self;
}
...
- (NSInteger)hidProfile {
    return [NSUserDefaults.standardUserDefaults integerForKey:kHIDProfileKey];
}

- (void)setHidProfile:(NSInteger)profile {
    if (profile < 0 || profile > 2) profile = 0;
    NSInteger previous = self.hidProfile;
    if (previous == profile) return;
    [NSUserDefaults.standardUserDefaults setInteger:profile forKey:kHIDProfileKey];

    {
        std::lock_guard<std::mutex> lock(_mutex);
        for (auto& pad : _wirelessPads) pad.reset();
        _wiredPad.reset();
        for (auto& slot : _wirelessSlots) {
    ...
    NSString* message = @"HID profile changed to Generic Joystick (1209:0360).";
    if (profile == 1) {
        message = @"HID profile changed to Xbox 360 Controller (045E:028E).";
    } else if (profile == 2) {
        message = @"HID profile changed to Xbox Series X Controller (045E:0B12).";
    }
    [self appendLog:message];
    [self notifyChanged];
}

- (NSArray<NSString*>*)virtualHIDDevices {
    std::lock_guard<std::mutex> lock(_mutex);
    return [_virtualHIDDevices copy];
}

- (void)refreshVirtualHIDDevices {
    ...
    if (set) {
        ...
        for (CFIndex i = 0; i < count; ++i) {
            IOHIDDeviceRef device = (IOHIDDeviceRef)values[static_cast<size_t>(i)];
            auto prop = ^NSString*(CFStringRef key) {
                CFTypeRef v = IOHIDDeviceGetProperty(device, key);
                if (!v || CFGetTypeID(v) != CFStringGetTypeID()) return @"";
                return (__bridge NSString*)v;
            };
            NSString* manufacturer = prop(CFSTR(kIOHIDManufacturerKey));
            NSString* product = prop(CFSTR(kIOHIDProductKey));
            ...
            if ([manufacturer rangeOfString:@"X360" options:NSCaseInsensitiveSearch].location == NSNotFound &&
                [product rangeOfString:@"Xbox 360" options:NSCaseInsensitiveSearch].location == NSNotFound &&
                [product rangeOfString:@"Xbox Series X" options:NSCaseInsensitiveSearch].location == NSNotFound &&
                [product rangeOfString:@"Receiver Bridge" options:NSCaseInsensitiveSearch].location == NSNotFound) continue;
            unsigned v = vid ? vid.unsignedIntValue : 0;
            unsigned p = pid ? pid.unsignedIntValue : 0;
            [found addObject:[NSString stringWithFormat:@"%@ — %@ — VID/PID %04X:%04X", manufacturer, product, v, p]];
        }
        CFRelease(set);
    }
    CFRelease(manager);
...
- (NSArray<X360ControllerSnapshot*>*)controllers {
    ...
    for (std::size_t i = 0; i < _wirelessSlots.size(); ++i) {
        ...
        [snapshots addObject:controllerSnapshot(
            ...
            static_cast<NSInteger>(i), NO, slot.headset_present, slot.state, slot.battery,
            slot.battery_candidate, slot.battery_update_count,
            slot.battery_pid_count,
    ...
    if (_wiredModel.connected) {
        ...
        [snapshots addObject:controllerSnapshot(
            ...
            _wiredModel.state, std::nullopt, std::nullopt, 0, 0,
            _wiredModel.hid_ready, _wiredModel.hid_failed,
...
- (void)handleWirelessEvent:(SlotEvent)event {
    ...
    std::string hidError;
    NSString* batteryLog = nil;
    bool sent0165Start = false;
    bool sent0165LedSequence = false;
    {
        std::lock_guard<std::mutex> lock(_mutex);
        ...
        if (event.connected.has_value()) {
            ...
            if (!slot.connected) {
                slot.state.reset();
                slot.battery.reset();
                slot.battery_candidate.reset();
                slot.battery_update_count = 0;
                slot.battery_pid_count = 0;
                slot.hid_ready = false;
                slot.hid_failed = false;
                slot.hid_message.clear();
                _wirelessPads[event.slot].reset();
            } else if (_receiver && !wasConnected) {
                // 0.16.5 waits for the initial 0x0f information packet before
                // WirelessHIDDevice::handleStart() sends weirdStart.
            }
        }
        if (event.battery_candidate.has_value()) {
            const bool firstInitialInfo = !slot.battery_candidate.has_value();
            slot.battery_candidate = *event.battery_candidate;
            if (firstInitialInfo && _receiver) {
                (void)_receiver->send_0165_weird_start(event.slot, nullptr);
                sent0165Start = true;
            }
        }
        if (event.battery_pid_packet) {
            ++slot.battery_pid_count;
        }
        if (event.battery.has_value()) {
            slot.battery = *event.battery;
            if (event.battery->dedicated_update) {
                ++slot.battery_update_count;
            }
            batteryLog = [NSString stringWithFormat:
                @"Slot %zu battery: %@", event.slot + 1,
                batterySummary(slot.battery, slot.battery_candidate,
                               slot.battery_update_count, slot.battery_pid_count)];
        }
        if (event.state.has_value()) {
            ...
            if (!_wirelessPads[event.slot]) {
                if (_receiver) {
                    (void)_receiver->set_led(
                        event.slot,
                        static_cast<std::uint8_t>(0x06 + event.slot),
                        nullptr);
                    (void)_receiver->send_0165_weird_start(event.slot, nullptr);
                    sent0165LedSequence = true;
                }

                VirtualGamepadOptions options;
                options.hid.profile =
                    self.hidProfile == 1 ? HidProfile::xbox360_controller :
                    (self.hidProfile == 2 ? HidProfile::xbox_series_x_controller
                                          : HidProfile::generic_joystick);
    ...
    if (sent0165Start) {
        [self appendLog:@"0.16.5 accurate startup: initial 0x0f received -> weirdStart sent."];
    }
    if (sent0165LedSequence) {
        [self appendLog:@"0.16.5 SetLEDs behavior: LED -> weirdStart sent."];
    }
    [self refreshVirtualHIDDevices];
    if (batteryLog) [self appendLog:batteryLog];
    [self notifyChanged];
}
...
- (void)send0165InitForControllerWithTag:(NSInteger)tag {
    if (tag == kWiredControllerTag) {
        [self appendLog:@"0.16.5 Init is available only for wireless Xbox 360 controllers."];
        return;
    }
    if (tag < 0 || tag >= 4) return;
    ...
    if (!receiver) {
        [self appendLog:@"0.16.5 Init: wireless receiver is not available."];
        return;
    }

    const std::size_t slot = static_cast<std::size_t>(tag);
    const std::uint8_t ledMode = static_cast<std::uint8_t>(0x06 + slot);
    std::string error;

    [self appendLog:@"0.16.5 Manual OUT #1 handleStart weirdStart: 00 00 00 40 00 00 00 00 00 00 00 00"];
    if (!receiver->send_0165_weird_start(slot, &error)) {
        [self appendLog:[NSString stringWithFormat:@"0.16.5 Init weirdStart #1 failed: %@", ns(error)]];
        return;
    }

    error.clear();
    [self appendLog:[NSString stringWithFormat:
        @"0.16.5 Manual OUT #2 SetLEDs LED: 00 00 08 %02x 00 00 00 00 00 00 00 00",
        static_cast<unsigned>(0x40u + (ledMode & 0x0fu))]];
    if (!receiver->set_led(slot, ledMode, &error)) {
        [self appendLog:[NSString stringWithFormat:@"0.16.5 Init LED failed: %@", ns(error)]];
        return;
    }

    error.clear();
    [self appendLog:@"0.16.5 Manual OUT #3 SetLEDs weirdStart: 00 00 00 40 00 00 00 00 00 00 00 00"];
    if (!receiver->send_0165_weird_start(slot, &error)) {
        [self appendLog:[NSString stringWithFormat:@"0.16.5 Init weirdStart #2 failed: %@", ns(error)]];
        return;
    }

    [self appendLog:@"0.16.5 manual sequence sent; watching for 00 00 00 13 XX battery updates."];
}

- (void)runBatteryHandshakeForControllerWithTag:(NSInteger)tag variant:(NSInteger)variant {
    if (tag == kWiredControllerTag) {
        [self appendLog:@"Battery Handshake Lab is available only for wireless Xbox 360 controllers."];
        return;
    }
    if (tag < 0 || tag >= 4) return;
    ...
    if (!receiver) {
        [self appendLog:@"Battery Handshake: wireless receiver is not available."];
        return;
    }
    ...
    auto doPresence = [&]() {
        [self appendLog:@"Battery Handshake OUT presence: 00 00 08 c0 00 00 00 00 00 00 00 00"];
        std::string error;
        const bool ok = receiver->send_presence_query(slot, &error);
        [self appendLog:ok
            ? @"Battery Handshake: presence query sent."
            : [NSString stringWithFormat:@"Battery Handshake: presence query failed: %@", ns(error)]];
        std::this_thread::sleep_for(std::chrono::milliseconds(150));
    };

    auto doLed = [&]() {
        [self appendLog:[NSString stringWithFormat:
            @"Battery Handshake OUT LED: 00 00 08 %02x 00 00 00 00 00 00 00 00",
            static_cast<unsigned>(0x40u + (ledMode & 0x0fu))]];
        std::string error;
        const bool ok = receiver->set_led(slot, ledMode, &error);
        [self appendLog:ok
            ? @"Battery Handshake: LED command sent."
            : [NSString stringWithFormat:@"Battery Handshake: LED command failed: %@", ns(error)]];
        std::this_thread::sleep_for(std::chrono::milliseconds(150));
    };

    switch (variant) {
        case 1:
            [self appendLog:@"Battery Handshake A: presence -> LED"];
            doPresence();
            doLed();
            break;
        case 2:
            [self appendLog:@"Battery Handshake B: LED -> presence"];
            doLed();
            doPresence();
            break;
        default:
            [self appendLog:@"Battery Handshake C: presence -> LED -> presence"];
            doPresence();
            doLed();
            doPresence();
            break;
    ...
    [self appendLog:@"Battery Handshake: sequence finished; watching incoming packets for ~2 seconds."];
}

- (void)powerOffControllerWithTag:(NSInteger)tag {
    if (tag == kWiredControllerTag) {
        [self appendLog:@"Power Off is available only for wireless Xbox 360 controllers."];
        return;
    }
    if (tag < 0 || tag >= 4) return;
    ...
    if (!receiver) {
        [self appendLog:@"Power Off failed: wireless receiver is not available."];
        return;
    }

    std::string error;
    if (!receiver->power_off(static_cast<std::size_t>(tag), &error)) {
        [self appendLog:[NSString stringWithFormat:@"Power Off failed for controller %ld: %@",
                         static_cast<long>(tag + 1), ns(error)]];
    } else {
        [self appendLog:[NSString stringWithFormat:@"Power Off command sent to controller %ld.",
                         static_cast<long>(tag + 1)]];
    }
}
