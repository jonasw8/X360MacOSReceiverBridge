...
std::vector<std::uint8_t> make_hid_report_descriptor(const HidOptions& options) {
    // All profiles deliberately use the same proven 14-byte input report
    // layout. Xbox profiles use Game Pad usage and differ in published HID
    // identity. This lets us test macOS compatibility without disturbing the
    // receiver/input path.
    const bool xbox_profile =
        options.profile == HidProfile::xbox360_controller ||
        options.profile == HidProfile::xbox_series_x_controller;
    const std::uint8_t usage = xbox_profile ? 0x05 :
        static_cast<std::uint8_t>(options.joystick_usage ? 0x04 : 0x05);

    std::vector<std::uint8_t> d = {
        0x05, 0x01,                         // Usage Page (Generic Desktop)
        0x09, usage,                        // Joystick or Game Pad
