...
struct ContentView: View {
    ...
    private func outputBadge(_ controller: X360ControllerSnapshot) -> some View {
        Text(controller.outputFailed ? "Needs Attention" : (controller.outputReady ? "Output Ready" : "Connected"))
        .font(.callout)
    ...
    private var controllerSubtitle: String {
        if model.controllers.isEmpty {
            return model.isScanning ? "Searching for Xbox 360 controllers." : "No connected controller."
        }
        return "Monitor and identify bridged Xbox 360 controllers."
    }

    private var discoveryDetail: String {
        model.isScanning ? "Listening for Xbox 360 wireless receivers and wired USB controllers." : "Ready to scan."
    }

    private var statusBadgeText: String {
        if model.connectedControllerCount > 0 {
            return "\(model.connectedControllerCount) Connected"
        }
        return model.isScanning ? "Scanning" : "Ready"
    }
...
struct StatusMenu: View {
    ...
    var body: some View {
        Text(model.connectedControllerCount == 0 ? "No controllers connected" : "\(model.connectedControllerCount) controller\(model.connectedControllerCount == 1 ? "" : "s") connected")
