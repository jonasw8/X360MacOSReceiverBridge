...
void Receiver::read_loop(std::size_t slot_index) {
    ...
    while (running_.load()) {
        int transferred = 0;
        const int rc = libusb_interrupt_transfer(handle_, slot.endpoint_in,
                                                  buffer.data(),
        ...
        if (options_.dump_raw) {
            log("slot " + std::to_string(slot_index + 1) + " raw: " +
                hex_dump(buffer.data(), static_cast<std::size_t>(transferred)));
        }

        const auto parsed = parse_wireless_packet(
        ...
        if (!parsed->connected.has_value() && !parsed->state.has_value()) {
            continue;
        }
        if (slot_callback_) {
            SlotEvent event;
