...
void test_battery_status() {
    std::array<std::uint8_t, 29> initial{};
    initial[0] = 0x00;
    initial[1] = 0x0f;
    initial[2] = 0x00;
    initial[3] = 0xf0;
    ...
    auto event = parse_wireless_packet(initial.data(), initial.size());
    CHECK(event.has_value());
    CHECK(event->battery_candidate.has_value());
    CHECK(*event->battery_candidate == 0x63);
    CHECK(event->battery.has_value());
    CHECK(event->battery->raw_status == 0x63);
    CHECK(!event->battery->dedicated_update);
    CHECK(event->battery->legacy_percentage() == 39);

    std::array<std::uint8_t, 29> update{};
    update[0] = 0x00;
    update[1] = 0x00;
    update[2] = 0x00;
    update[3] = 0x13;
    ...
    event = parse_wireless_packet(update.data(), update.size());
    CHECK(event.has_value());
    CHECK(!event->battery_candidate.has_value());
    CHECK(event->battery.has_value());
    CHECK(event->battery->raw_status == 0xa2);
    CHECK(event->battery->dedicated_update);
    CHECK(event->battery->legacy_percentage() == 64);

    std::array<std::uint8_t, 8> pid{};
    pid[0] = 0x00;
    pid[1] = 0x09;
    event = parse_wireless_packet(pid.data(), pid.size());
    CHECK(event.has_value());
    CHECK(event->battery_pid_packet);
}
...
void test_0165_weird_start() {
    const auto weird = make_0165_weird_start();
    CHECK(weird[0] == 0x00);
    CHECK(weird[1] == 0x00);
    CHECK(weird[2] == 0x00);
    CHECK(weird[3] == 0x40);
    for (std::size_t i = 4; i < weird.size(); ++i) CHECK(weird[i] == 0x00);
}

int main() {
    test_0165_weird_start();
    try {
        test_presence();
        test_input_decode();
